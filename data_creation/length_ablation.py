# Length ablation study - only HPC and HPCE instances with HPC repeated twice
import re
import os
import sys
import random
...
# Only HPC and HPCE for length ablation
CONTEXT_TYPES = ["HPC", "HPCE"]
...
def contextual_knowledge_tasks(raw_dataset, version_name):
    ...
    def create_ck_instance(example):
        for context_type in CONTEXT_TYPES:
            # For HPC, repeat the context twice
            if context_type == "HPC":
                context = example[f"{context_type}_context"] + " " + example[f"{context_type}_context"]
            else:
    ...
    # Write to local
    os.makedirs(os.path.join(os.path.join(os.environ["data_dir"], "length_ablation_task")), exist_ok=True)
    processed_dataset.to_json(os.path.join(os.environ["data_dir"], "length_ablation_task", f"{model_name}_contextual_knowledge_{version_name}.jsonl"))
...
def parametric_knowledge_tasks(raw_dataset, version_name):
    ...
    # Write to local
    os.makedirs(os.path.join(os.path.join(os.environ["data_dir"], "length_ablation_task")), exist_ok=True)
    processed_dataset.to_json(os.path.join(os.environ["data_dir"], "length_ablation_task", f"{model_name}_parametric_knowledge_{version_name}.jsonl"))
    return processed_dataset
...
def parametriccontextual_knowledge_tasks(raw_dataset, version_name):
    ...
    # Write to local
    os.makedirs(os.path.join(os.path.join(os.environ["data_dir"], "length_ablation_task")), exist_ok=True)
    processed_dataset.to_json(os.path.join(os.environ["data_dir"], "length_ablation_task", f"{model_name}_parametriccontextual_knowledge_{version_name}.jsonl"))
    return processed_dataset
...
def rag_task(raw_dataset, version_name):
    ...
    # Write to local
    os.makedirs(os.path.join(os.path.join(os.environ["data_dir"], "length_ablation_task")), exist_ok=True)
    processed_dataset.to_json(os.path.join(os.environ["data_dir"], "length_ablation_task", f"{model_name}_rag_{version_name}.jsonl"))
    return processed_dataset
